!(function( global, factory ) {
	...
	var MicFlow = function(id, property) {
		...
		if (MicFlow.remarks.headBtnsList) {
			for(var i=0;i<MicFlow.remarks.headBtnsList.length;++i){
				headToolsBtn += "<a href='javascript:void(0)' class='MicFlowHeadBtn' id='ico_"+ MicFlow.remarks.headBtnsList[i] +"' title=''><i class='ico_"+ MicFlow.remarks.headBtnsList[i] +"'></i></a>";
			}
	...
	return MicFlow.prototype = {
		...
		intBtn: function() {
			this.headArea.on("mousedown","[id^=ico_]", function(e){
				if(!e) e = window.event;
				if(e.button === 2) return false;

				var classname = e.target.id ? e.target.id : e.target.className;

				if (classname) {
					$('#'+ classname).css("background-color", "#98bf21");
				}
